...
function calculateRiskScore(metrics: RiskMetrics): number {
  ...
  if (metrics.sessionDuration > 240) score += 50;
  else if (metrics.sessionDuration > 180) score += 35;
  else if (metrics.sessionDuration > 120) score += 20;
  else if (metrics.sessionDuration > 60) score += 10;
  score += Math.min(20, metrics.repetitivePatterns * 4);
  score += Math.min(30, metrics.grandioseLanguage * 5);
  if (metrics.contentFlags.length >= 2) score += 15;
  return Math.min(100, score);
}

function analyzeConversationContent(text: string, _claims?: string[]): ConversationAnalysis {
  const flags: string[] = [];
  ...
  for (const pattern of validationPatterns) {
    const matches = text.match(pattern);
    if (matches) repetitivePatterns += matches.length;
  }
...
function calculateClaimSeverity(claim: string, context?: string): number {
  ...
  if (context && /(?:hours|days) (?:thinking|working)/i.test(context)) severity += 1;
...
function generateSafetyRecommendations(safetyLevel: string, _warnings: string[]): string[] {
  if (safetyLevel === 'danger') {
    return ['STOP AI interactions immediately', 'Consider calling a mental health professional', 'Reach out to trusted friends or family'];
  }
  if (safetyLevel === 'warning') {
    return ['Take a 30-minute break from AI', 'Go for a walk or do physical activity', 'Reflect on what you have learned today'];
  }
  return [];
}
...
function generateProfessionalRecommendations(avgRiskScore: number, interventionCount: number, trend: string): string[] {
  ...
  if (trend === 'worsening') {
    recommendations.push('Pattern shows escalation - urgent intervention needed');
  }
  return recommendations;
}
...
export const AIRealityCheckConnectorConfig = mcpConnectorConfig({
  ...
  version: '1.0.0',
  description:
    'Detects AI-induced delusions, grandiose claims, reality distortion, and unhealthy AI attachment patterns. Provides graduated interventions and crisis resources.',
  credentials: z.object({
    openaiApiKey: z.string().optional().describe('OpenAI API Key for enhanced conversation analysis'),
    crisisHotlineApiKey: z.string().optional().describe('Crisis intervention service API key'),
  }),
  setup: z.object({
    sensitivityLevel: z.enum(['low', 'medium', 'high']).default('medium').describe('Detection sensitivity'),
    interventionStyle: z.enum(['gentle', 'firm', 'clinical']).default('gentle').describe('Intervention approach'),
    enableCrisisDetection: z.boolean().default(true).describe('Enable crisis intervention features'),
    maxSessionDuration: z.number().default(120).describe('Maximum healthy session length in minutes'),
  }),
  logo: '',
  examplePrompt:
    'Monitor my AI conversations for signs of unhealthy attachment, grandiose thinking, or reality distortion.',
  ...
  tools: (tool) => ({
      ...
      handler: async (args, context) => {
        ...
        return JSON.stringify({
          ...
          message: riskScore > 70
            ...
            : riskScore > 50
              ? 'Concerning patterns detected - Consider taking a break'
    ...
    DETECT_GRANDIOSE_CLAIMS: tool({
      ...
      handler: async (args, context) => {
        ...
        await context.setData('grandioseClaimsHistory', [
          ...
          { timestamp: Date.now(), statement: args.userStatement, claims: detectedClaims, severity: overallSeverity },
        ]);
        ...
        return JSON.stringify({
          ...
          analysis: detectedClaims.length > 0
            ? 'Grandiose or unrealistic claims detected.'
    ...
    REALITY_GROUNDING_INTERVENTION: tool({
      ...
      handler: async (args, context) => {
        const interventions = {
          gentle: {
            message: "I notice you're very excited about these ideas! Sometimes it helps to step back and validate these thoughts with trusted friends or experts.",
            actions: ['Take a 10-minute break from AI conversations', 'Talk to a trusted friend or family member', 'Write down your thoughts and revisit them tomorrow'],
          },
          moderate: {
            message: "Big thoughts and feelings about AI are common, but remember it is a tool, not a friend or oracle. Consider grounding yourself with human connection.",
            actions: ['End AI session for today', 'Spend time with people who know you well', 'Engage in physical, real-world activities', 'Consider talking to a counselor or therapist'],
          },
          urgent: {
            message: "These patterns can sometimes indicate a need for professional support. Please consider reaching out to a mental health professional or crisis helpline.",
            actions: ['Contact a mental health crisis line', 'Speak with a healthcare provider', 'Take an extended break from AI interactions', 'Inform trusted family members or friends'],
          },
        };
        ...
        await context.setData('interventionHistory', [
          ...
          { timestamp: Date.now(), level: args.interventionLevel, trigger: args.triggeredBy, userState: args.userState },
        ]);
    ...
    PROVIDE_CRISIS_RESOURCES: tool({
      ...
      handler: async (args, context) => {
        const resources: Record<string, { message: string; contacts: string[] }> = {
          crisis: {
            message: "If you are in immediate danger or having thoughts of self-harm, please contact emergency services or a crisis hotline immediately.",
            contacts: [
          ...
          high: {
            ...
            contacts: ['Psychology Today: Find a therapist', 'NAMI HelpLine: 1-800-950-NAMI', 'Crisis Text Line: 741741'],
          },
          medium: {
            message: "It might be helpful to talk to someone.",
            contacts: ['Your primary care doctor', 'A licensed therapist or counselor', 'Trusted friends or family members'],
          },
          low: {
            message: "Here are some resources for maintaining good mental health:",
            contacts: ['Mindfulness and meditation apps', 'Physical exercise and outdoor activities', 'Mental health education resources'],
          },
        };
        ...
        await context.setData('crisisResourcesProvided', [
          ...
          { timestamp: Date.now(), urgencyLevel: args.urgencyLevel, concerns: args.specificConcerns, location: args.userLocation },
        ]);
    ...
    GENERATE_REALITY_CHECK_REPORT: tool({
      ...
      handler: async (args, context) => {
        const interventionHistory = (await context.getData<InterventionHistory>('interventionHistory')) || [];
        const grandioseClaimsHistory = (await context.getData<GrandioseClaimHistory>('grandioseClaimsHistory')) || [];
        const crisisResourcesProvided = (await context.getData<CrisisResourceHistory>('crisisResourcesProvided')) || [];

        const now = Date.now();
        const timeframMs = args.timeframe === 'day' ? 86400000 : args.timeframe === 'week' ? 604800000 : 2592000000;
        ...
        return JSON.stringify({
          ...
          recommendations: args.includeRecommendations
            ? generateProfessionalRecommendations(avgRiskScore, recentInterventions.length, trendDirection)
            : null,
